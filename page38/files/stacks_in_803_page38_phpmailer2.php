...
function unicode_decode($str){
	return preg_replace_callback("/[\\%]u([0-9A-F]{4})/i",function($match){
		return iconv('utf-16','utf-8',hex2str($match[1]));
	}, $str);
}
...
$fromName     = $mustache->render(decode_string('Joe Schmoe'), $data);
$fromAddress  = $mustache->render('no-reply@server.com', $data);

$replyName    = $mustache->render(decode_string('Joe Schmoe'), $data);
$replyAddress = $mustache->render('joe@server.com', $data);

$toName       = $mustache->render(decode_string('Joe Schmoe'), $data);
$toAddress    = $mustache->render('you@domain.com', $data);
...
try {
	...
	$mail->Body = stripslashes(newline2break($body));

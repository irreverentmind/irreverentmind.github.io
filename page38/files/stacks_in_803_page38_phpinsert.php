...
function unicode_decode($str){
	return preg_replace_callback("/[\\%]u([0-9A-F]{4})/i", function($matches){
		iconv('utf-16', 'utf-8', hex2str($matches[1]));
	}, $str);
}
...
try {
  $db_conn = new PDO("mysql:host=$dbServer;port=$dbPort;dbname=$dbName;charset=utf8", $dbUsername, $dbPassword); 
}
catch(PDOException $e) {
	return_error('Could not connect to database server: '.$e->getMessage());
}
catch(Exception $e) {
...
// Execute the query
try {
	$query = $db_conn->prepare("INSERT INTO $dbTable (". implode(',',$keys) .") value (:". decode_string(implode(',:',$keys)) .")");
	$query->execute($_POST);
}
catch(PDOException $e) {
	return_error("Problem inserting data: ".$e->getMessage());
}

return_success("Insert Success!");
$db_conn = null;

...
class Mailer
{
    ...
    public function __construct($options)
    {
        ...
        $this->adminSend    = $options['adminSend'];
        ...
        $this->toName       = $this->templateString($options['toName']);
        $this->toAddress    = $this->templateString($options['toAddress']);
        $this->toName2      = $this->templateString($options['toName2']);
        $this->toAddress2   = $this->templateString($options['toAddress2']);
        $this->subject      = $this->templateString($options['subject']);

        $this->replyTo      = $options['replyTo'];
        $this->secondTo     = $options['secondTo'];
        $this->secondBCC    = $options['secondBCC'];
    ...
    {
        $body = $this->template ? $this->templateBody() : $this->defaultBody();

        // Normalize body line breaks
        $body     = $this->mail->normalizeBreaks($body);
        $bodyText = strip_tags($this->break2newline($body));
        ...
        try {
            ...
            $this->mail->addAddress($this->toAddress, $this->toName);
            //Add second address if desired & bcc if set
            if ($this->secondTo) {
                if ($this->secondBCC) {
                    $this->mail->AddBCC($this->toAddress2, $this->toName2);
                } else {
                    $this->mail->addAddress($this->toAddress2, $this->toName2);
                }
            }
            //Set the subject line
            $this->mail->Subject = $this->subject;
            //Replace the plain text body with one created manually
            $this->mail->Body    = stripslashes($this->newline2break($body));
            $this->mail->AltBody = $bodyText;

            // Add the following HTML to the form for file uploads to work...
            // <input type="file" name="file">
            if (isset($_FILES['file']) && $_FILES['file']['error'] == UPLOAD_ERR_OK) {
                $dir = $_SERVER["DOCUMENT_ROOT"].'/uploads';
                $fullpath = "$dir/".$_FILES['file']['name'];
                if (!file_exists($dir)) {
                    mkdir($dir);
                }
            ...
            // Fallback to mail()
            if (mail($this->toAddress, $this->subject, $bodyText)) {
    ...
    public function postData()
    {
        ...
        if (isset($_POST["jwtemplate"])) {
            $this->template = $_POST["jwtemplate"];
            unset($_POST["jwtemplate"]);
        }
    ...
    private function templateBody()
    {
        ...
        $template = preg_replace('/<!--(.*?)-->/', '', preg_replace('/^ +/m', '', urldecode($this->template)));
        $body = $this->mustache->render($template, $this->postData);
        return $this->decodeString($body);
    }

    public function returnError($msg)
    {
        if ($this->emailAdmin) {
            $mail->setFrom($this->adminFrom);
            $mail->addAddress($this->adminTo);
